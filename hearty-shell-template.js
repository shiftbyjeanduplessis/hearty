(function(){
  'use strict';
  var VERSION='hearty-production-guard-v29-nav-final';
  var NAV_PAGES=[
  ...
  function normaliseNav(){
    ...
    if(!/^(home|meals|exercise|progress|support|social|settings)$/.test(active)){
      if(active==='login') document.querySelectorAll('#bottomNav,[data-shell-mount="bottom-nav"],#heartyEmergencyNav,.hearty-emergency-nav,[data-emergency-nav]').forEach(function(el){el.remove();});
      return;
    }
  ...
  window.HeartyProductionGuardV29={normaliseNav:normaliseNav,syncSupportState:syncSupportState,version:VERSION};
})();
