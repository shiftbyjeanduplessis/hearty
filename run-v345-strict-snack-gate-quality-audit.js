...
function qaScenario(input, result){
  ...
  if(result.version !== "3.4.5-strict-snack-gate") hard.push("wrong engine version");
  if(!String(result.engineSource || "").includes("v345-strict-snack-gate")) hard.push("wrong/missing engine source");
...
fs.writeFileSync("hearty_v345_strict_snack_gate_quality_results.json", JSON.stringify({gate, fullResults, invalidResults}, null, 2));
fs.writeFileSync("hearty_v345_strict_snack_gate_full_plans.md", fullPlans.join("\n"));
