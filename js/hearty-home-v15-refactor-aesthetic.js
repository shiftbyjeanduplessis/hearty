(function(){
  ...
  function renderMedication(){
    // Home no longer owns the injection reminder card. Injection rhythm is displayed on Support.
    const titleEl = $('medicationTitle');
    const card = $('injectionCard');
    const btn = $('logDoseBtn');
    const copy = $('nextInjectionDateText');
    if(!titleEl || !card || !btn || !copy) return;
    ...
    card.classList.toggle('due', due && !logged);
    card.classList.toggle('logged', logged);
    btn.classList.toggle('active', logged);

    if (!med.type || med.frequency === 'set_later') {
      copy.textContent = 'Medication rhythm can be managed later from Support or Settings.';
    } else if (med.type === 'Not using medication yet' || med.frequency === 'not_using') {
      copy.textContent = 'Medication reminders are off for now.';
    } else if (logged) {
      copy.textContent = 'Logged for today.';
    } else if (due) {
      copy.textContent = 'Planned for today.';
    } else {
      copy.textContent = medicationScheduleCopy(med);
    }
  }
  ...
  function bindTasks(){
    ...
    const doseBtn = $('logDoseBtn');
    if(doseBtn) doseBtn.addEventListener('click',()=>{
      const today = localDate();
      ...
      try{
        ...
        const row = { id:'dose-' + today, date:today, medication:med, source:'home', createdAt:new Date().toISOString() };
