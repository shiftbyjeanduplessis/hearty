# Perfect Women Local Tracker V1.3.3 — Technical Spec
...
## V1.2.8 — Beginner strength programs

Added a stripped-down version of the Hearty exercise programme inside Programs:

- Home beginner strength programme
- Gym beginner strength programme
- Minimal cards by default; active programme expands only when selected
- Current programme banner supports walking, jogging, home strength and gym strength
- Exercise cards with images, targets, simple technique cues and completion logging
- Strength summary added to Progress
- Hearty exercise images copied into `/perfect-women/images/exercises/`

Data remains local-first and uses the same local storage key, so existing users should keep their data.


## V1.2.8 audit notes
- Audited bottom navigation and all page targets: Home, Track, Programs, Progress, Photos, Recipes and Settings.
- Added cross-page deep links so buttons can open the correct card/panel, not only the page.
- Program shortcut buttons can now focus Walking or Jogging inside Programs.
- Compact program cards now preview first, then start from the expanded panel.
- Added visual panel highlight after cross-page navigation.
- JavaScript syntax and service-worker asset paths were checked.


## V1.2.9 easy wins update

- Added a Home "Next action" card that chooses the most useful next step: Monday weigh-in, water, movement, photo reminder, measurement reminder, weekly check-in, or progress review.
- Added a 4-week progress rhythm reminder for photos and measurements.
- Added recipe favourites with a Favourites filter, heart buttons on recipe cards, and a save/unsave option inside the recipe modal.
- Improved recipe search to include title, tags, summary, ingredients, method, plan notes and search terms.
- Updated PWA cache name to `perfect-women-tracker-v1.3.0-foundation-player`.


## V1.3.0 — Interactive Foundation Home Program
- Replaces the basic Home Beginner Strength card with a four-week Foundation Home Program.
- Weeks 1–2: three guided foundation sessions per week.
- Weeks 3–4: four timed/circuit workouts per week.
- Includes exercise countdowns, workout clock, rest timers, low-impact alternatives, pause/resume, previous/skip, saved workout position and completed-session history.
- Reuses the existing local client storage and `homeStrength` current-program key, so existing app data remains compatible.


## V1.3.1 — Program open/preview fix
- Fixed a bug where opening Foundation Home while another programme was current only showed a toast and left the card collapsed.
- Explicitly opened programme previews now expand ahead of the current-program card without changing saved current-program state.
- Added automatic smooth scrolling and a visual highlight to the programme card after opening.
- Starting Foundation Home or Gym Strength now returns the client to the expanded active programme.
- Updated PWA cache to `perfect-women-tracker-v1.3.1-program-open-fix`.


## V1.3.2 — Program focus cleanup

- The active/current program is always moved to the top and remains fully expanded.
- Inactive programs are reduced to one slim row with only title, short status and Start/Switch.
- Starting an inactive program now makes it current immediately instead of opening a competing preview.
- Updated PWA cache to `perfect-women-tracker-v1.3.2-program-focus-cleanup`.


## V1.3.3 — Hearty-style progressive overload engine

The strength-player layer now reuses and adapts the set-by-set interaction pattern from the Hearty exercise system.

### Workout flow
- Client opens the current programme and taps **Start workout** or **Resume workout**.
- Each strength exercise opens as three progressive sets:
  1. warm-up set at approximately 60% of the planned working weight
  2. build-up set at approximately 80%
  3. working set at the planned working weight
- Client taps **Go** to start the set stopwatch.
- At the end of the set, the client logs actual weight and reps.
- A rest countdown runs before the next set.
- The completed workout saves every exercise and every set.

### Progressive overload
The final working set determines the next-session recommendation:
- reaching the top of the rep range with a weighted exercise increases the next working weight
- falling below the minimum rep range keeps the same weight and asks the client to rebuild reps
- finishing inside the range keeps the same weight until the top of the range is achieved
- gym increments are 5 kg for leg press/squat and 2.5 kg for other machines
- home weighted exercises increase by 1 kg
- qualifying bodyweight exercises progress after two top-range completions

### Persistence
- Active home and gym workouts are stored separately.
- Set number, current exercise, logged sets, rest countdown and elapsed time are resume-safe.
- The existing local storage key remains unchanged.
- Existing client logs and programme history remain compatible.

### PWA cache
- Updated to `perfect-women-tracker-v1.3.3-progressive-overload-engine`.
