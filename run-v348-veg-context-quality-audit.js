...
function qaScenario(input, result){
  ...
  for(const d of days){
    ...
    if (/frittata|omelette|scrambled|tofu scramble/.test(`${b} ${dinner}`) && /green beans|butternut|lettuce|cabbage|cauliflower|broccoli/.test(`${b} ${dinner}`)) {
      hard.push(`Day ${d.day}: egg/tofu breakfast-dinner uses bad egg veg`);
    }

    if (hotMealWords.test(dinner) && (hasWord(dinner,"lettuce") || hasWord(dinner,"cucumber"))) hard.push(`Day ${d.day}: raw veg in cooked dinner`);
    if (/soup|stew|chilli|cooked/.test(lunch) && (hasWord(lunch,"lettuce") || hasWord(lunch,"cucumber"))) hard.push(`Day ${d.day}: raw veg in cooked lunch`);
