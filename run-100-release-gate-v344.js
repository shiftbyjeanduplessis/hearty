...
function extraQA(input, result){
  ...
  if(result.version !== "3.4.4-release-gate-repair") hard.push("Wrong engine version");
  if(!result.engineSource || !String(result.engineSource).includes("v344-100gen-gate-passed-candidate")) hard.push("Missing/incorrect engine source");
...
fs.writeFileSync("hearty_v344_100_gate_results.json", JSON.stringify({gate, results}, null, 2));
fs.writeFileSync("hearty_v344_100_gate_full_plans.md", full.join("\n"));
