(function(){
  "use strict";
  var VERSION = "hearty-production-guard-v15-critical-restore";
  ...
  window.heartySetSupportMode = window.heartySetSupportMode || function(reason, source){
    var payload = { active:true, isActive:true, reason:reason||null, type:reason||'', sourcePage:source||'guard', updatedAt:Date.now(), updated_at:nowIso() };
    safeSetJSON('hearty_support_mode_v1', payload); safeSetJSON('heartySupportState', payload); safeSetJSON('hearty_support_state', payload); safeSetJSON('heartySupportMode', payload); safeSet('supportMode','on'); if(reason){ safeSet('meals_support_mode', reason); } safeSet('hearty_support_mode','on'); safeSet('heartySupportActive','true');
    try { window.dispatchEvent(new CustomEvent('hearty:support-mode-changed',{ detail:payload })); } catch(_) {}
  };
  ...
  function bindNavLinkNormalizer(){
    document.addEventListener('click', function(ev){
      var a = ev.target && ev.target.closest && ev.target.closest('a[data-nav-target]');
      if (!a) return;
      var target = a.getAttribute('data-nav-target');
      var map = { home:'/home.html', meals:'/meals.html', exercise:'/exercise.html', progress:'/progress.html', support:'/support.html', social:'/social.html', settings:'/settings.html' };
      if (map[target]) {
        ev.preventDefault();
