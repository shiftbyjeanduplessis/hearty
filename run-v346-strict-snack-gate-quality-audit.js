...
const VEG = {
  US:["spinach","tomato","onion","carrot","zucchini","green beans","mushrooms","peppers","broccoli","cucumber","lettuce","cauliflower","butternut"],
  CA:["spinach","tomato","onion","carrot","zucchini","green beans","mushrooms","peppers","broccoli","cucumber","lettuce","cauliflower","butternut"],
  AU:["spinach","tomato","onion","carrot","zucchini","green beans","mushrooms","peppers","broccoli","cucumber","lettuce","cauliflower","butternut"],
  UK:["spinach","tomato","onion","carrot","courgette","green beans","mushrooms","peppers","broccoli","cucumber","lettuce","cauliflower","butternut"],
  SA:["spinach","tomato","onion","carrot","baby marrow","green beans","mushrooms","peppers","broccoli","cucumber","lettuce","cauliflower","butternut"]
};
...
function validScenario(i) {
  ...
  const common = {
    name:`v345 valid quality scenario ${i+1} ${region} mode ${mode}`,
    region,
    vegetables: VEG[region].slice(0, 8 + (i % 5)),
  ...
  const cases = [
    {diet:"omnivore", proteins:["chicken","beef","pork","fish","eggs","dairy","protein_powder"], breakfastItems:["eggs",yoghurt,"oats","protein_shake"], snackProteins:[yoghurt,"boiled_eggs","tuna",jerky,"protein_shake","chicken_strips"]},
    ...
    {diet:"vegetarian", proteins:["eggs","dairy","tofu","lentils","beans","chickpeas"], breakfastItems:["eggs",yoghurt,"tofu_scramble","hummus_plate"], snackProteins:[yoghurt,"boiled_eggs","hummus","tofu"]},
    ...
    {diet:"omnivore", proteins:["chicken","beef","eggs","dairy","protein_powder"], breakfastItems:["eggs",yoghurt,"oats"], snackProteins:[yoghurt,"boiled_eggs",jerky,"chicken_strips","protein_shake"]},
    {diet:"pescatarian", proteins:["fish","eggs","dairy","protein_powder"], breakfastItems:["eggs",yoghurt,"oats"], snackProteins:[yoghurt,"boiled_eggs","tuna","protein_shake"]},
    ...
    // formerly broken narrow snack scenario: now add enough snacks to be valid
...
function invalidSnackScenario(i) {
  ...
  const yoghurt = y(region);
  const common = {
    name:`v345 invalid snack gate ${i+1}`,
    region,
    diet:"omnivore",
  ...
  const cases = [
    {snackProteins:[yoghurt,"boiled_eggs"]},
    {snackProteins:["protein_shake","boiled_eggs"]},
    {snackProteins:[yoghurt,"protein_shake"]},
    {snackProteins:["boiled_eggs"]},
    {snackProteins:[yoghurt]},
    {snackProteins:["protein_shake"]},
    {snackProteins:["boiled_eggs",yoghurt]},
    {snackProteins:["boiled_eggs","protein_shake"]},
    {snackProteins:[yoghurt,"protein_shake"]},
    {snackProteins:["boiled_eggs",yoghurt]}
  ];
  return {...common, ...cases[i % cases.length]};
}
...
function isBreakfastVegBad(meal){
  const t = mealText(meal);
  if(!(hasAnyWord(t, ["egg","eggs"]) || /omelette|scrambled|tofu scramble/.test(t))) return false;
  return hasAnyWord(t, ["cauliflower","broccoli","cabbage","lettuce","butternut"]) || t.includes("green beans");
}
function renderPlan(s,result){
  const lines = [];
  lines.push(`# ${s.index}. ${s.name}`, "");
  lines.push(`Engine: ${result.version}`);
  lines.push(`Source: ${result.engineSource}`);
  lines.push(`Region: ${s.region}`);
  lines.push(`Diet: ${s.diet}`);
  lines.push(`Status: ${result.status}`);
  lines.push(`Engine QA: ${result.qa?.status}`);
  if(result.qa?.hard?.length) lines.push(`Engine hard: ${result.qa.hard.join(" | ")}`);
  if(result.qa?.warn?.length) lines.push(`Engine warn: ${result.qa.warn.join(" | ")}`);
  lines.push("");
  for(const d of result.days || []){
    ...
    for(const [label, m] of [["Breakfast",d.breakfast],["Morning snack",d.morningSnack],["Lunch",d.lunch],["Afternoon snack",d.afternoonSnack],["Dinner",d.dinner]]){
      lines.push(`**${label}**`);
      lines.push(`${m.name} Approx. ${m.protein} protein`);
      lines.push(`Basic method: ${m.detail}`, "");
    }
...
function qaScenario(input, result){
  ...
  if(result.version !== "3.4.6-strict-snack-gate-quality-fix") hard.push("wrong engine version");
  if(!String(result.engineSource || "").includes("v346-strict-snack-gate-quality-fixed")) hard.push("wrong/missing engine source");
  ...
  for(const d of days){
    ...
    if(!input.noBreakfast && /lentil|bean|chickpea/.test(b) && !/hummus breakfast plate/.test(b)) hard.push(`Day ${d.day}: legume breakfast bowl`);
    if(isBreakfastVegBad(d.breakfast)) hard.push(`Day ${d.day}: non-breakfast veg in breakfast`);
  ...
  const snackFamilies = (input.snackProteins || []).map(x => {
  ...
  for(const d of days){
    ...
    const bf = classifyMeal(d.breakfast);
    for(const snack of [d.morningSnack,d.afternoonSnack]){
      const sc = classifyMeal(snack);
      if(["eggs","dairy","protein_powder"].includes(sc) && sc === bf) hard.push(`Day ${d.day}: ${sc} breakfast/snack repeat`);
      const alternatives = snackFamilies.filter(x => !main.has(x) && x !== bf);
      if(sc !== "fruit" && main.has(sc) && alternatives.length >= 2) weird.push(`Day ${d.day}: ${sc} snack/main repeat despite alternatives`);
    }
  }

  if(/with no added starch/.test(text)) weird.push("clunky no-starch title");
  const cottageCount = (text.match(/cottage cheese protein plate/g)||[]).length;
  if(cottageCount > 3) weird.push(`cottage cheese lunch appears ${cottageCount} times`);
...
fs.writeFileSync("hearty_v346_strict_snack_gate_quality_results.json", JSON.stringify({gate, fullResults, invalidResults}, null, 2));
fs.writeFileSync("hearty_v346_strict_snack_gate_full_plans.md", fullPlans.join("\n"));
